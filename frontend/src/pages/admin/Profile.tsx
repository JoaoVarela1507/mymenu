...
export default function Profile() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    getRestaurantByOwnerId(user.id).then(setRestaurant);
  }, [user?.id]);
        ...
        <div className="max-w-2xl">
          <Card>
            ...
            <div className="space-y-4">
              ...
              {restaurant && (
                <div>
                  <label className="block text-sm font-semibold text-dark mb-1">Restaurante</label>
                  <p className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700">
                    {restaurant.logo || '🍽️'} {restaurant.restaurantName}
